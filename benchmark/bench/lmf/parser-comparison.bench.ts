  ...
  [
    ...
    describe(name, () => {
      ...
      Object.entries(PARSER_REGISTRY).forEach(([parserKey, parserInfo]) => {
        bench(`${(parserInfo as any).name} - ${name}`, async () => {
          const parser = (parserInfo as any).factory();
          await parser.parse(await getOrCreateLmfFile(slug, numWords, numSynsets));
        });
      });
      // Add wn-pybridge LMF load benchmark
      bench(`wn-pybridge LMF load - ${name}`, async () => {
        let bridge;
        try {
          bridge = new WnBridge();
          await bridge.init();
          await bridge.lmf.load(await getOrCreateLmfFile(slug, numWords, numSynsets));
        } catch (error) {
          console.warn('wn-pybridge LMF load failed:', error);
        } finally {
...
async function createLmfFile(
  ...
  try {
    const testDir = join(tmpdir(), 'wordnet-benchmark-lmf');
