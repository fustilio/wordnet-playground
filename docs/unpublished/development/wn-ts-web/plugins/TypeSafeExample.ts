/**
 * Type-Safe Plugin Example
 * Demonstrates type-safe plugin usage with WordNet kernel
 */
...
// Example usage
async function demonstrateTypeSafeUsage() {
  try {
    console.log('=== Type-Safe Plugin Demo ===\n');
    
    // Type-safe WordNet operations
    const words = await wordnet.getWord('test');
    console.log('Words found:', words.length);
    
    console.log('\n=== Demo Complete ===');
  } catch (error) {
    console.error('Type-safe plugin error:', error);
  }
}
