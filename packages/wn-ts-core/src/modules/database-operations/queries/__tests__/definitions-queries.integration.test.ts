...
createIntegrationTestSuite('Definitions Queries Integration Tests', (getContext: () => IntegrationTestContext) => {
  ...
  it('should return empty array for non-existent synset', async () => {
    ...
    const query = getDefinitionsBySynsetIdQuery(context.kyselyDb, nonExistentSynsetId);
    const results = await query.execute();
  ...
  it('should handle concurrent definition queries', async () => {
    ...
    const queries = synsetIds.map(id => getDefinitionsBySynsetIdQuery(context.kyselyDb, id));
