...
describe('Examples Queries Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    tempDbPath = join(testContext.tempDir, 'test-examples-queries.db');
  ...
  describe('getExamplesBySynsetIdQuery', () => {
    it('should find examples by synset ID', async () => {
      const synsetId = 'test-computer-n-1';
      
      const query = getExamplesBySynsetIdQuery(kyselyDb, synsetId);
      const results = await query.execute();
    ...
    it('should find multiple examples for a synset', async () => {
      const synsetId = 'test-computer-n-1';
      ...
      expect(Array.isArray(results)).toBe(true);
      expect(results.length).toBe(2); // We have 2 examples for this synset
    ...
    it('should return empty array for synset with no examples', async () => {
    ...
    it('should return examples with correct structure', async () => {
      ...
      if (results.length > 0 && results[0]) {
        ...
        expect(example).toHaveProperty('synset_id');
    ...
    it('should handle synset with single example', async () => {
      const synsetId = 'test-run-v-1';
      ...
      expect(Array.isArray(results)).toBe(true);
      expect(results.length).toBe(1); // We have 1 example for this synset
      
      if (results[0]) {
        expect(results[0].synset_id).toBe(synsetId);
        expect(results[0].text).toContain('run');
      }
    });
  });
  ...
  describe('Real Data Integration', () => {
    it('should work with actual WordNet data structure', async () => {
      ...
      if (lexicons.length > 0) {
        ...
        if (synsets.length > 0) {
          ...
          // Note: mockCore doesn't have getExamples method, so we test the query directly
          if (synset?.id) {
            const query = getExamplesBySynsetIdQuery(kyselyDb, synset.id);
    ...
    it('should handle database constraints correctly', async () => {
      ...
      const query = getExamplesBySynsetIdQuery(kyselyDb, 'test-computer-n-1');
      const results = await query.execute();
    ...
    it('should verify database integrity', async () => {
      ...
      expect(examples.length).toBe(3); // 2 for computer-n-1, 1 for run-v-1
