...
export class Morphy {
  ...
  private async initializeFromWordnet(): Promise<void> {
    ...
    try {
      ...
      // If database is not available (e.g., in wn-ts-core), 
      // just skip initialization and continue with uninitialized state
      console.warn('Morphy initialization skipped: database not available');
    }
 ...
 */
export function createMorphy(wordnet?: WordNetKernel): Morphy {
