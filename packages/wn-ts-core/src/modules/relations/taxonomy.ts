...
import type { Synset, PartOfSpeech, SynsetQuery } from '../../core/types.js';
    ...
    const hasHyponyms = synsets.some((other: any) => {
      return (
        ...
        other.relations.some(
          (rel: any) => rel.type === 'hypernym' && rel.target === synset.id
        )
      );
    });
 ...
 */
export async function hypernymPaths(
  synset: Synset,
  wordnet: WordNetCore,
  simulateRoot = false
): Promise<Synset[][]> {
 ...
 */
export async function minDepth(synset: Synset, wordnet: WordNetCore): Promise<number> {
