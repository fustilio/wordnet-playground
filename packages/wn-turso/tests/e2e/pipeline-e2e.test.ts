...
const synsetData: Synset[] = [
  { id: 'oewn-00001740-n', pos: 'n', language: 'en', definition: 'A machine for computing', ili: 'i1' },
  { id: 'oewn-00002740-n', pos: 'n', language: 'en', definition: 'A device that processes data', ili: 'i2' },
  { id: 'oewn-00003740-v', pos: 'v', language: 'en', definition: 'To calculate mathematically', ili: 'i3' },
  { id: 'omw-th-00001-n', pos: 'n', language: 'th', definition: 'เครื่องคอมพิวเตอร์', ili: 'i1' },
  { id: 'omw-fr-00001-n', pos: 'n', language: 'fr', definition: 'Une machine pour calculer', ili: 'i1' },
  { id: 'oewn-00004740-a', pos: 'a', language: 'en', definition: 'Related to computation' },
  { id: 'oewn-00005740-n', pos: 'n', language: 'en', definition: 'Electronic device for information' },
];

const wordData: Word[] = [
  { id: 'w1', lemma: 'computer', pos: 'n', language: 'en', synset_id: 'oewn-00001740-n' },
  { id: 'w2', lemma: 'calculator', pos: 'n', language: 'en', synset_id: 'oewn-00001740-n' },
  { id: 'w3', lemma: 'compute', pos: 'v', language: 'en', synset_id: 'oewn-00003740-v' },
  { id: 'w4', lemma: 'คอมพิวเตอร์', pos: 'n', language: 'th', synset_id: 'omw-th-00001-n' },
  { id: 'w5', lemma: 'ordinateur', pos: 'n', language: 'fr', synset_id: 'omw-fr-00001-n' },
];
...
describe('E2E: Pipeline Workflows', () => {
  describe('Source of Truth -> Working DB Pattern', () => {
    it('should transfer synsets with extended working columns', async () => {
      ...
      const result = await Pipeline
        .from(arraySource(synsetData, 'source-turso:synsets'))
    ...
    it('should handle multilingual filtering for translation DB', async () => {
      ...
      const result = await Pipeline
        .from(arraySource(synsetData, 'source:synsets'))
  ...
  describe('Data Migration Workflows', () => {
    it('should migrate and transform schema', async () => {
      ...
      const result = await Pipeline
        .from(arraySource(synsetData, 'old-schema:synsets'))
        .map(row => ({
          synset_id: row.id,
  ...
  describe('Analytics Pipeline', () => {
    it('should compute aggregates via batching', async () => {
      ...
      const batchStats: { batchNum: number; count: number; avgDefLen: number }[] = [];
      let batchNum = 0;

      await Pipeline
        .from(arraySource(synsetData, 'analytics:synsets'))
        .batch(3)
        .tap(batchRows => {
          batchNum++;
          const avgDefLen = batchRows.reduce((sum, r) => sum + r.definition.length, 0) / batchRows.length;
          batchStats.push({
            batchNum,
  ...
  describe('Join-like Operations', () => {
    it('should enrich synsets with word counts', async () => {
      ...
      const result = await Pipeline
        .from(arraySource(synsetData, 'synsets'))
        .filter(row => row.language === 'en')
        .extend(row => ({
  ...
  describe('Error Handling', () => {
    it('should handle empty source gracefully', async () => {
      ...
      const result = await Pipeline
        .from(arraySource([], 'empty:synsets'))
        .filter(() => true)
        .to(sink);
  ...
  describe('Performance Characteristics', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      const result = await Pipeline
        .from(arraySource(largeData, 'large:synsets'))
        .filter(row => row.language === 'en')
        .filter(row => row.pos === 'n')
