  ...
  const saveToCache = useCallback(async (
    ...
    try {
      ...
      const fileHandle = await (root as any).getFileHandle(filename, { create: true });
       ...
       // Prefer synchronous access when available, otherwise fallback to stream write
       const supportsSync = typeof (fileHandle as any).createSyncAccessHandle === 'function';
       ...
       if (supportsSync) {
         ...
         try {
           for (let offset = 0; offset < uint8Array.length; offset += chunkSize) {
             ...
             if (progress) progress(written / uint8Array.length);
           }
         ...
         // Fallback: write via standard writable stream
