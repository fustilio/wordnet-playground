...
export interface WordNetWorkerAPI {
  ...
  loadPackageFromData(packageId: string, data: ArrayBuffer): Promise<{
    success: boolean;
    data?: any;
    error?: string;
  }>;
...
export class WordNetWorkerClient {
  ...
  constructor() {
    console.log('WordNetWorkerClient created');
  }
  ...
  private async _initialize(workerUrl: string | URL): Promise<boolean> {
    try {
      console.log('Initializing WordNet worker');
      ...
      this.remote = wrap<WordNetWorkerAPI>(this.worker);
      ...
      console.log('WordNet worker initialized successfully');
      this.emit('initialized', { success: true });
      
      return true;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('Failed to initialize WordNet worker', error);
  ...
  async getStatus(): Promise<any> {
    ...
    try {
      ...
      console.log('Status from worker', result);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('Failed to get status', { error: errorMessage });
  ...
  private async updateLexiconTracking(): Promise<void> {
    ...
    try {
      ...
      console.warn('Failed to update lexicon tracking', { error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  async queryWords(term: string, pos?: string): Promise<any[]> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`Failed to query words for term: ${term}`, { error: errorMessage });
  ...
  async querySynsets(term: string, pos?: string): Promise<any[]> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`Failed to query synsets for term: ${term}`, { error: errorMessage });
  ...
  async querySenses(term: string, pos?: string): Promise<any[]> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`Failed to query senses for term: ${term}`, { error: errorMessage });
  ...
  async testMemoryQueries(): Promise<any> {
    ...
    try {
      console.log('Testing memory queries');
      const result = await this.remote!.testMemoryQueries();
      
      if (result.success) {
        console.log('Memory test completed successfully', { data: result.data });
        return result.data;
      } else {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('Failed to run memory test', { error: errorMessage });
