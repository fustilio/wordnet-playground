  ...
  describe("Combined Stress Tests", () => {
    ...
    it("should handle memory pressure scenarios", async () => {
      ...
      try {
        ...
        expect(statsResults.length).toBe(10);
        expect(lexiconResults.length).toBe(10);
      } finally {
        // Clean up instances
  ...
  describe("Error Recovery Tests", () => {
    it("should recover from SQLITE_NOMEM errors gracefully", async () => {
      ...
      const promises: Promise<
        ...
        | { error: string }
      >[] = [];
      const numCalls = 50; // High number to potentially trigger memory issues
      ...
      expect(results).toHaveLength(numCalls * 2);
